...
dtree <- rpart(formula=noshow ~ age_decade +
                 ...
                 solbor + sandum + sancla + sancec + itarar + lourde + cabral + quadro + horto + penha + jesus + cypres + sanmar +
                 sms + morning + daybw + noprevpct,
               data = zf,
               method = "class",
               control = rpart.control(xval = 5, cp=0.0001, minsplit=100),
               )
rpart.plot(dtree)
